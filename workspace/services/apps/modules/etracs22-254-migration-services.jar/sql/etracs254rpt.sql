...
[findBldgRpuById2]
select * from bldgrpu where objid = $P{rpuid}
...
INSERT INTO rpu_assessment (objid, rpuid, classification_objid, actualuse_objid, areasqm, areaha, marketvalue, assesslevel, assessedvalue, rputype,taxtable) 
values($P{objid}, $P{rpuid}, $P{classification_objid}, $P{actualuse_objid}, $P{areasqm}, $P{areaha}, $P{marketvalue}, $P{assesslevel}, $P{assessedvalue}, $P{rputype},$P{taxtable})
...
[findRpuByFaasId]
select * from faas where objid = $P{faasid}

[findEntityById]
select objid from entity where objid = $P{objid}

[fixtitleno]
UPDATE faas SET titletype = $P{titletype}, titleno = $P{titleno},titledate = $P{issuedate} WHERE objid = $P{objid}

[getRpusForRecalc]
SELECT * FROM faas WHERE rpuid NOT IN (SELECT objid FROM etracs254_recalculated_faas)
LIMIT ${count}

[logRecalculatedFaas]
INSERT INTO etracs254_recalculated_faas (objid) VALUES ($P{rpuid})
